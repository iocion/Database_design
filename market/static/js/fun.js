...
class TextScramble {
  ...
  if (playButton) {
      ...
      console.error('playButton 未找到');
  }
  const glowInput = document.getElementById('glowIntensity');
    if (glowInput) {
        glowInput.addEventListener('input', updateGlow); // 使用 'input' 事件实时更新
    } else {
        console.error('glowIntensity 未找到');
    }
    ...
    if (settingsInput) {
        settingsInput.addEventListener('input', updateSettings); // 使用 'input' 事件实时更新
    } else {
        console.error('settingsInput 未找到');
    }
